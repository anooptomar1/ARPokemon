...
class ViewController: UIViewController {
  ...
  func createCaptureSession() -> (session: AVCaptureSession?, error: NSError?) {
    ...
    if backVideoDevice != nil {
      ...
      if error == nil {
        ...
        error = NSError(domain: "", code: 1, userInfo: ["description": "Error creating capture device."])
      }
    } else {
  ...
  func getHeadingForDirectionFromCoordinate(from: CLLocation, to: CLLocation) -> Double {
    ...
    let degree = radiansToDegrees(atan2(sin(tLng-fLng)*cos(tLat), cos(fLat)*sin(tLat)-sin(fLat)*cos(tLat)*cos(tLat)*cos(tLng-fLng)))
